...
func RegisterV2(app *pocketbase.PocketBase) {
	app.RootCmd.AddCommand(&cobra.Command{
		...
		Run: func(command *cobra.Command, args []string) {
			...
			err := app.Dao().RunInTransaction(func(txDao *daos.Dao) error {
				...
				println("Parsing levels & records")
				var levelNames []string
				err = readFileIntoJson(path+"/_list.json", &levelNames)
				if err != nil {
					return err
				}
				levelsToGenerate := make([]Level, len(levelNames))
				usersToGenerate := make(map[string]bool)
				for position, levelName := range levelNames {
					...
					levelsToGenerate[position - 1] = level
					usersToGenerate[strings.ToLower(level.Verifier)] = true
					usersToGenerate[strings.ToLower(level.Author)] = true

					for _, creator := range level.Creators {
						usersToGenerate[strings.ToLower(creator)] = true
					}
					for _, playerRecord := range level.Records {
						usersToGenerate[strings.ToLower(playerRecord.User)] = true
					}
				}
				println("Genrating users, levels and records")
				userIdMap := make(map[string]string)
				for username, _ := range usersToGenerate {
					password := util.RandString(20)
					usedName := util.RandString(10)
					_, err := txDao.DB().Insert(names.TableUsers, dbx.Parms{
						"username": usedName,
