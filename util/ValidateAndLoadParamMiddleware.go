...
func LoadParam(toLoad LoadData) echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			if err != nil {
				return NewErrorResponse(err, "Failed to load form data")
			}
...
func loadWithConversion[T any](converter func(string) (T, error), conversionError string, required bool, rules ...validation.Rule) LoadFunc {
	return func(key string, params map[string][]string) (interface{}, error) {
		if _, exists := params[key]; !exists {
			if required {
				return nil, NewErrorResponse(nil, fmt.Sprintf("%s can't be empty", key))
			} else {
				return nil, nil
			}
		}
		if len(params[key]) != 1 {
			return nil, NewErrorResponse(nil, fmt.Sprintf("%s has to hold exactly one value", key))
		}
		value, err := converter(params[key][0])
		if err != nil {
			return nil, NewErrorResponse(nil, fmt.Sprintf("%s%s", key, conversionError))
		}
		err = validation.Validate(value, rules...)
		if err != nil {
			return nil, NewErrorResponse(nil, fmt.Sprintf("%s: %s", key, err.Error()))
		}
		return value, nil
	}
}
...
func LoadStringArray(required bool, rules ...validation.Rule) LoadFunc {
	return func(key string, params map[string][]string) (interface{}, error) {
		...
		if err != nil {
			return nil, NewErrorResponse(nil, fmt.Sprintf("%s: could not parse string array", key))
		}
